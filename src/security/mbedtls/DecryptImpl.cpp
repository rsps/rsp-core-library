...
struct MbedTLSDecrypt : public MbedTLSCryptBase
{
    ...
    void Update(const uint8_t *apData, size_t aSize) override
    {
        ...
        auto rc = mbedtls_cipher_update(&mCipherCtx, apData, aSize, mData.current(), &out_len);
        if (rc) {
    ...
    SecureBuffer Finalize() override
    {
        ...
        auto rc = mbedtls_cipher_finish(&mCipherCtx, mData.current(), &out_len);
...
{
    return std::unique_ptr<CryptBase>(new MbedTLSDecrypt(aCipher));
}
